...
const [target, outputArg, secondsArg = '20'] = process.argv.slice(2);
...
try {
  ...
  manifest = { captured_at: new Date(started).toISOString(), requested_url: target, final_url: page.url(), http_status: response.status(), title: await page.title(), width: 1280, height: 720, duration_seconds: duration, method: 'Actual browser screenshots at approximately 1 Hz, encoded using elapsed wall-clock intervals; no UI replacement or time acceleration.', audio: 'none; NOT a completed submission video', limitations: 'Public page capture only; does not verify Graph queries, model training, or ENS deployment.', video_sha256: createHash('sha256').update(await readFile(resolve(output, 'capture.mp4'))).digest('hex') };
