...
Check the requested block and recompute the hash of the newly generated bytes; after upload, that
run's stored hash must match its predicted hash. Row notes include collection timestamps, so a fresh
run need not have the historical file's SHA-256 even at the same block. Compare the underlying facts
separately from collection metadata. Uploading, training, payment and publishing are separate actions.
